...
本研究では、類似度算出処理(\ref{subsec:similarity_calculation}節で後述)において、要求仕様とテストケースの類似度を、Jaccard係数を用いて算出する。
...
\begin{itemize}
    ...
    \item 各要求仕様に固有の要求仕様IDを割り当てる。要求仕様IDは、要求仕様IDの命名規則に従っている。
\end{itemize}
...
\begin{description}
    ...
    要求仕様記述項目は、6つの項目と項目ごとの記述ルールで定義する。
...
\begin{itemize}
    ...
    \item 各テストケースに固有のテストケースIDを割り当てる。テストケースIDは、テストケースIDの命名規則に従っている。
\end{itemize}
...
\begin{itemize}
    ...
    \item 対応付け可能な要求仕様\\
    要求仕様の対応付け要素 (「前提条件」、「トリガー」、「システム応答」)の値が全て空文字列でない要求仕様を意味する。\\
    要求仕様の対応付け要素のうち、いずれか$1$つでも対応付け要素の値が空文字列である要求仕様は、テストケースとの対応付けの対象外とする。
