...
\begin{itemize}
    ...
    \item http.server \\
    http.server モジュールは、簡易的なHTTPサーバーを構築するための機能を提供するモジュールである。
    主に、ローカル環境上での開発や検証を目的として、静的ファイルの配信やHTTP通信の動作確認を容易に行うことができる。
    本研究では、TCVダイアグラム出力処理 (\cref{subsec:diagram-generation-process}節で後述)において、
    SVG形式として生成したTCVダイアグラム(\cref{sec:testcase_visualization_diagram}で後述)を配信するローカルサーバーの起動に使用する。
\end{itemize}
...
PlantUML \cite{plantuml} は、テキストデータから UML \cite{uml} のダイアグラムや UML 以外の様々なタイプのダイアグラムを生成できるツールである。
...
Jaccard係数は、2つの集合の類似度を測る指標の1つである\cite{jaccard}。テキストに含まれる単語を集合の要素と見なすことで、テキスト間の類似度を算出できる。
...
以降、要求仕様テンプレートに基づいて記述した要求仕様の内容を、要求仕様文と呼ぶ。

\section{\tool{}の入力対象}\label{sec:input}

本研究で試作する\tool{}は、要求仕様書(\ref{subsec:reqspec}節で後述)とテストスイート(\ref{subsec:testsuite}節で後述)を入力とし、
TCVダイアグラム(\ref{sec:testcase_visualization_diagram}節で後述)を出力する。

\subsection{要求仕様書}\label{subsec:reqspec}
\tool は、以下の8つの条件を全て満たす要求仕様書を入力対象とする。

\begin{itemize}
    \item 日本語で記述する。
    \item CSVファイル(.csv)として記述する。
    \item 機能要求を対象とする。
    \item 各要求仕様は、要求仕様記述項目(後述)の定義に従って記述する。
    \item 「前提条件」および「システム応答」の項目は、2個以上の記述がある場合、読点(、)で区切って記述する。
    \item 各要求仕様に固有の要求仕様IDを割り当てる。
    \item 要求仕様IDは、要求仕様IDの命名規則(後述)に従う。
    \item 要求仕様書フォーマット(後述)に従って記述する。
\end{itemize}

\tool の入力対象である要求仕様書の例を、図\ref{fig:reqspec_example}に示す。

\begin{figure}[tp]
        \centering
        \fbox{
            \includegraphics[width=0.96\textwidth]{images/02/reqspec_example.png}
            }
            \caption{\tool の入力対象である要求仕様書の例}
            \label{fig:reqspec_example}
        \end{figure}
...
\begin{description}
    ...
    \item[要求仕様書フォーマット]\mbox{}\\
    要求仕様書のCSVファイルは、以下の形式で記述する。
    \begin{itemize}
        \item 1行目は、要求仕様記述項目(表\ref{tb:reqspec_item}参照)の各項目をカンマ(,)区切りで並べた、
        ヘッダー行(要求仕様ID,要求仕様名,前提条件,トリガー,システム応答)とする。
        \item 2行目以降に各要求仕様をヘッダー行の形式に従って、1行ずつ記述する。
    \end{itemize}
\end{description}

\subsection{テストスイート}\label{subsec:testsuite}
\tool は、以下の9つの条件を全て満たすテストスイートを入力対象とする。
...
\begin{itemize}
    ...
    \item 各テストケースは、テストケース記述項目(後述)の定義に従って記述する。
    \item 「事前条件」、「操作」、および、「期待出力」の項目は、2個以上の記述がある場合、読点(、)で区切って記述する。
    \item 各テストケースは1つの要求仕様のみをテストの対象とし、同一の要求仕様をテストするテストケースは複数存在しない。
    \item 各テストケースには、入力対象の全てのテストスイート間で、固有のテストケースIDを割り当てる。
    \item テストケースIDは、テストケースIDの命名規則(後述)に従う。
    \item テストスイートフォーマット(後述)に従って記述する。
\end{itemize}

\tool の入力対象であるテストスイートの例を、図\ref{fig:testsuite_example}に示す。

\begin{figure}[tp]
        \centering
        \fbox{
        \includegraphics[width=0.96\textwidth]{images/02/testsuite_example.png}
        }
        \caption{\tool の入力対象であるテストスイートの例}
        \label{fig:testsuite_example}
    \end{figure}
...
\begin{table}[tp]
    ...
    \begin{tabular}{c|l|l}
        ...
        \hline
        事前条件 & テストを実行する前に、システムが満たしているべき状態や環境 & 1個以上 (必須) \\
        \hline
        操作 & テスト実行時に行う具体的な操作 & 1個以上 (必須) \\
        \hline
        期待出力 & 操作の結果として想定されるシステムの出力 & 1個以上 (必須) \\
        \hline
    \end{tabular}
\end{table}
...
\begin{description}
    ...
    テストケースIDは、TC\texttt{-}から始まり、その後に一意の番号が続く形式であり、要求仕様IDで定義した階層関係は導入しない。
    なお、テストケースIDは、入力対象の全てのテストスイート間で一意となるように割り当てる。

    \item[テストスイートフォーマット]\mbox{}\\
    テストスイートのCSVファイルは、以下の形式で記述する。
    \begin{itemize}
        \item 1行目は、テストケース記述項目(表\ref{tb:testcase_item}参照)の各項目をカンマ(,)区切りで並べた、
        ヘッダー行(テストケースID,テストケース名,事前条件,操作,期待出力)とする。
        \item 2行目以降に各テストケースをヘッダー行の形式に従って、1行ずつ記述する。
    \end{itemize}
\end{description}

\section{要求仕様とテストケースの対応関係に関する用語}\label{sec:words}
本研究で用いる、要求仕様とテストケースの対応関係に関する用語を、以下に示す。

\begin{itemize}
    \item 対応関係\\
    特定のテストケースが、特定の要求仕様をテストする関係を、対応関係と呼ぶ。
    ...
    \item テストケースの充足\\
    ある要求仕様を適正にテストするテストケースが存在する状態を、テストケースの充足と呼ぶ。
    \item テストケースの過不足\\
    以下のテストケースの抜け、テストケースの不備、テストケースの過剰の3つのテストケースの状態を総称して、テストケースの過不足と呼ぶ。
    \begin{itemize}
        \item テストケースの抜け\\
        ある要求仕様をテストするテストケースが存在しない状態を、テストケースの抜けと呼ぶ。
        \item テストケースの不備\\
        ある要求仕様をテストするテストケースは存在するが、テストケースの内容が要求仕様に対して不十分な状態を、テストケースの不備と呼ぶ。
        \item テストケースの過剰\\
        テストケースは存在するが、テストする要求仕様が存在しない状態を、テストケースの過剰と呼ぶ。
    \end{itemize}
\end{itemize}
