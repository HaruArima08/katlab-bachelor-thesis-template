...
\tool は、以下の8つの条件をすべて満たす要求仕様書を入力対象とする。
...
\begin{figure}[tp]
        \centering
        \fbox{
            \includegraphics[width=0.96\textwidth]{images/02/reqspec_sample1.png}
            }
...
\tool は、以下の9つの条件をすべて満たすテストスイートを入力対象とする。
...
\begin{itemize}
    ...
    \item 各テストケースには、入力対象のすべてのテストスイート間で、固有のテストケースIDを割り当てる。
...
\begin{figure}[tp]
        \centering
        \fbox{
        \includegraphics[width=0.96\textwidth]{images/02/tc_2.png}
        }
...
\begin{description}
    ...
    なお、テストケースIDは、入力対象のすべてのテストスイート間で一意となるように割り当てる。
