...
\begin{description}
    ...
    要求仕様記述項目のうち、「前提条件」、「トリガー」、「システム名」、および「システム応答」の項目は、 \ref{sec:EARS}節で述べた要求仕様テンプレートの各要素に対応している。
    要求仕様記述項目は、6つの項目と項目ごとの記述ルールで定義する。
    なお、「前提条件」および「システム応答」の項目は、2個以上の記述がある場合、読点(、)で区切って記述する。
    一方、「トリガー」の項目は0または1個であるため、テキスト中に読点を含む場合でも1個のトリガーとして扱う。
    また、前提条件、トリガー、システム名は、それぞれ要求仕様記述項目の1つとして定義しているが、省略可能な項目である。特に、システム名の項目に記述がなかった場合、初期設定(\ref{sec:config}節で後述)で設定ファイルに記述しているシステム名を用いる。
...
\begin{itemize}
    ...
    \item 入力対象とする、すべてのテストスイートにおいて、各テストケースに固有のテストケースIDを割り当てている。
