...
\begin{itemize}
    ...
    \item 機能要求を対象とする。
    \item 各要求仕様は、要求仕様記述項目(表\ref{tb:reqspec_item}参照)の定義に従って記述する。
    \item 「前提条件」および「システム応答」の項目は、2個以上の記述がある場合、読点(、)で区切って記述する。
    \item 各要求仕様に固有の要求仕様IDを割り当てる。
    \item 要求仕様IDは、要求仕様IDの命名規則に従う。
    \item 要求仕様書フォーマットに従って記述する。
\end{itemize}
...
なお、本研究では、複数のテストケースの集合をテストスイートとする。

\begin{itemize}
    \item 日本語で記述する。
    \item CSVファイル(.csv)として記述する。
    \item ファイル名が、「テストスイート名.csv」である。
    \item 各テストケースは、テストケース記述項目(表\ref{tb:testcase_item}参照)の定義に従って記述する。
    \item 各テストケースは1つの要求仕様のみを検証対象とし、同一の要求仕様を検証するテストケースは複数存在しない。
    \item 各テストケースには、入力対象のすべてのテストスイート間で、固有のテストケースIDを割り当てる。
    \item テストケースIDは、テストケースIDの命名規則に従う。
    \item テストスイートフォーマットに従って記述する。
\end{itemize}
...
\begin{description}
    ...
    \begin{itemize}
        \item 1行目は、テストケース記述項目(表\ref{tb:reqspec_item}参照)の各項目をカンマ(,)区切りで並べた、
